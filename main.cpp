...
int main(int argc, char *argv[])
{
    ...
    qtTranslator.load("qt_" + langName, QLibraryInfo::path(QLibraryInfo::TranslationsPath));
    a.installTranslator(&qtTranslator);

    // For those languages that come splitted, load only the base Qt module translation
    QTranslator qtBaseTranslator;
    qtBaseTranslator.load("qtbase_" + langName, QLibraryInfo::path(QLibraryInfo::TranslationsPath));
